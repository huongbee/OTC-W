...
describe('Trade', () => {
  test('Without Ads > Throw Error', async (done) => {
    ...
    const countActive = await AdsModel.countDocuments({ levelAllowed: 3 });
    if (countActive > 0) {
  ...
  test('Exists Ads >>Trade success', async (done) => {
    ...
    if (countActive <= 0) {
      ...
      expect(resultLevel2Login.code).equal(1000);
      ...
      expect(createAds.code).equal(1000);
      adsId = createAds.data.id;
    ...
    if (countActive <= 0 && adsId !== 0) {
      ...
      expect(cancelAds.code).equal(1000);
    }
    done();
  });
  afterAll(async (done) => {
